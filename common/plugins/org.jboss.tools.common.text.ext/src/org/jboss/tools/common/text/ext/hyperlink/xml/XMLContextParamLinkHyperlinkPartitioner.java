...
public abstract class XMLContextParamLinkHyperlinkPartitioner extends AbstractHyperlinkPartitioner implements IHyperlinkPartitionRecognizer {
	 ...
	 */
	protected IHyperlinkRegion parse(final IDocument document, final int offset, final IHyperlinkRegion superRegion) {
		return StructuredModelWrapper.execute(document,
				new ICommand<IHyperlinkRegion>() {
					@Override
					public IHyperlinkRegion execute(IDOMDocument xmlDocument) {
						try {

							if (!recognize(document, offset, superRegion))
								return null;
							IRegion r = getRegion(document, xmlDocument, offset);
							if (r == null)
								return null;
							...
							return new HyperlinkRegion(r.getOffset(), r
									.getLength(), axis, contentType, type);
	...
	private IRegion getRegion(IDocument document, Document xmlDocument, int offset) throws BadLocationException {
		if (xmlDocument == null)
			return null;
